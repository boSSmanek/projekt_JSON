...
class Application(tk.Tk):
    def __init__(self):
        ...
        export_button_to_csv = ttk.Button(
            button_frame, text="Generate CSV", command=self.export_csv_file
        )
        export_button_to_csv.pack(side=tk.LEFT, padx=5, pady=5)

        # Combobox for selected powers
        self.selected_powers_combobox = ttk.Combobox(
            button_frame, values=self.selected_powers, state="readonly"
        )
        self.selected_powers_combobox.pack(
            side=tk.LEFT, padx=5, pady=5, fill=tk.X, expand=True
        )
        self.selected_powers_combobox.bind(
            "<<ComboboxSelected>>", self.display_selected_json
        )
    ...
    def open_json_file(self, selected_power=None):
        ...
        if file_path:
            try:
                with open(file_path, "r", encoding="utf-8") as file:
                    data = json.load(file)
                    if selected_power:
                        self.power_data[selected_power] = data
                    else:
    ...
    def select_or_add_power(self):
        ...
        def on_select():
            ...
            if selection:
                ...
                if selected_power not in self.selected_powers:
                    self.selected_powers.append(selected_power)
                    self.selected_powers_combobox["values"] = self.selected_powers
                messagebox.showinfo("Selected Power", f"You selected: {selected_power}")
                power_window.destroy()  # Zamknięcie okna po wyborze
                self.open_json_file(selected_power)
            else:
        ...
        add_button = tk.Button(
            power_window, text="Add New Power", command=add_new_power
        )
    ...
    def load_powers_from_file(self):
        try:
            ...
            messagebox.showerror(
                "Error", "Error reading powers file. Please check the file format."
            )
    ...
    def display_selected_json(self, event):
        selected_power = self.selected_powers_combobox.get()
        self.tree.delete(*self.tree.get_children())
        self.text.delete(1.0, tk.END)

        if selected_power in self.power_data:
            power_data = self.power_data[selected_power]
            self.populate_tree_view(power_data)

            self.text.delete(1.0, tk.END)
        else:
            self.text.delete(1.0, tk.END)
            self.text.insert(tk.END, f"No data available for power: {selected_power}")
    ...
    def export_csv_file(self):
        ...
        if not selected_power:
            messagebox.showerror("Error", "No power selected!")
            return

        folder_path = filedialog.askdirectory(title="Choose a folder location")
        if not folder_path:
            return

        folder_path = os.path.join(folder_path, selected_power)
        os.makedirs(folder_path, exist_ok=True)
        file_path = os.path.join(folder_path, "IntegerMessage.csv")
